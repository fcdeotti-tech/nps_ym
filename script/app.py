...
def exibir_tamanho_amostra(df):
    ...
    if 'Subcausa da nota de recomendação' in df_calc.columns:
        df_calc = df_calc[~df_calc['Subcausa da nota de recomendação'].astype(str).str.upper().isin(['TOTAL DA CAUSA', 'TOTAL'])]
    if 'Região' in df_calc.columns:
        df_calc = df_calc[~df_calc['Região'].astype(str).str.upper().isin(['TOTAL NACIONAL', 'TOTAL'])]
    if 'Grupo' in df_calc.columns:
        df_calc = df_calc[~df_calc['Grupo'].astype(str).str.upper().str.startswith('TOTAL -')]
    if 'Concessionária' in df_calc.columns:
        df_calc = df_calc[~df_calc['Concessionária'].astype(str).str.upper().str.startswith('TOTAL -')]
    if 'Modelo' in df_calc.columns:
